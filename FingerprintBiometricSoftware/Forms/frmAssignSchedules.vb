...
Public Class frmAssignSchedules
    ...
    Dim mDepartment_ID As Integer = 0
    ...
    Private Sub Clear_Fields()
        ...
        With cboSchedules
            ...
            If NetOpen(mSchedules, "select 0 schedule_id,'No Schedule' description " & _
                                    "Union All " & _
                                    "select a.* from (select schedule_id,description from schedules " & _
                                    "order by description) a", Cn.Connection) = True Then
                .HoldFields()
    ...
    Private Sub btnBrowseEmployee_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBrowseEmployee.Click
        ...
        If mSearchID > 0 Then
            ...
            If NetOpen(dtEmployees, "SELECT CONCAT(lastname,', ',firstname,' ',middlename) description,pay_type,department_id FROM employees where employee_id =" & mSearchID) = True Then
                If dtEmployees.Rows.Count > 0 Then
                    For Each drEmployee As DataRow In dtEmployees.Rows
                        ...
                        mDepartment_ID = CType(drEmployee.Item("department_id"), Integer)
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
            ...
            With tdbAssignSchedules
                If .RowCount > 0 Then
                    Do While mRow <= .RowCount - 1
                        mCommand.CommandText = "insert into schedules_assignment(employee_id,schedule_id,work_date,department_id) values (" & _
                                    "" & CType(txtEmployeeName.Tag, Integer) & "," & _
                                    "" & CType(.Item(mRow, "schedule_id"), Integer) & "," & _
                                    "'" & Format(CType(.Item(mRow, "work_date"), Date), "yyyy-MM-dd") & "'," & mDepartment_ID & ")"
                        mCommand.ExecuteNonQuery()
        ...
        Finally
            ...
            Finally
                ComputeDTR(CType(txtEmployeeName.Tag, Integer), CType(Mid(txtCutoffPeriod.Text, 1, 10), Date), CType(Mid(txtCutoffPeriod.Text, 14, 10), Date), mDepartment_ID)
    ...
    Private Sub btnGenerateDTR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGenerateDTR.Click
        ...
        If NetOpen(dtEmployees, "select a.employee_id,b.department_id from " & _
                                "(select employee_id from employee_dtrs where work_date BETWEEN '" & Format(mDateFrom, "yyyy-MM-dd") & "' AND '" & Format(mDateUntil, "yyyy-MM-dd") & "' group by employee_id) a " & _
                                "left outer join employees b on a.employee_id=b.employee_id") Then
            If dtEmployees.Rows.Count > 0 Then
                For Each mEmpIDs As DataRow In dtEmployees.Rows
                    ComputeDTR(CType(mEmpIDs.Item("employee_id"), Integer), CType(Mid(txtCutoffPeriod.Text, 1, 10), Date), CType(Mid(txtCutoffPeriod.Text, 14, 10), Date), CType(mEmpIDs.Item("department_id"), Integer))
                Next
            End If
    ...
    Private Sub tdbAssignSchedules_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tdbAssignSchedules.Click
